...
public class PerformanceCheckUtils {
    ...
    public static String getChatCompletionResult(String selectedText, Project project) {
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(),selectedText + "\n" + CUSTOM_PROMPT);
        ChatCompletionRequest request = new ChatCompletionRequest();
        // list content support update
        request.setMessages(new ArrayList<>() {{ add(chatMessage); }});
        ...
        try {
            ...
            for (String noPerformanceIssue : NO_PERFORMANCE_ISSUES) {
                if (response.contains(noPerformanceIssue)) {
                    return selectedText;
                }
            }
            return response;
    ...
    public static void showDiff(Project project, Editor editor, VirtualFile originalFile, Document replaceDocument) {
        ...
        DiffRequest diffRequest = new SimpleDiffRequest("HiCodeAssistant: Diff view",
                replaceContent, originalContent, "HiCodeAssistant suggested code", originalFile.getName() + "(original code)");
